                    ...
                    {resultArray.map(movie => 
                        <div class="col-md-4 text-center">
                            <div class="card mb-4 box-shadow">
                                {movie.poster_path && movie.poster_path ? <img class="card-img-top" src={`https://image.tmdb.org/t/p/w500/${movie.poster_path}`} alt="something" /> : <img class="card-img-top" src='https://www.kindpng.com/picc/m/18-189751_movie-placeholder-hd-png-download.png'  /> }
                            
                            <div class="movieDesc"> 
                                <h4>{movie.title}</h4>
                                <i class="fas fa-star" style={{color:"#ffeb3b"}}></i>{movie.vote_average}
                                    <div class="d-flex justify-content-center">
                                        <Link to={"/movieDetails/" + movie.id }>
                                            <button type="button" class="btn btn-outline-primary mr-2">View More</button>
                                        </Link>
                                        <button type="button" onClick={() => getMovieId("watchlist", movie)} class="btn btn-outline-primary mr-2">Add to Watchlist</button>
                                        <button type="button" onClick={() => getMovieId("favourites", movie)} class="btn btn-outline-primary mr-2"><i class="far fa-heart"></i></button>
                                    </div>
                                </div>
                            </div>
                        </div>
